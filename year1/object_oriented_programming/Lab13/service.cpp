...
void Service::updateContract(string nume, int ore, string tip, string prof) {
	if (nume == "")
		throw RepoException("Nume invalid!");
	if (ore <= 0)
		throw RepoException("Nr ore invalid!");
	...
	Disciplina d{ nume, ore, tip, prof };
	try {
		this->contract.findRepo(nume);
		this->undoList.push_back(make_unique<UpdateUndo>(contract, repo.findRepo(nume)));
		this->contract.updateRepo(d);
	}
