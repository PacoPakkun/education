...
public class UtilizatorDbRepository implements Repository<Long, Utilizator> {
    ...
    public Iterable<Utilizator> findFriends(Long id) {
        Set<Utilizator> friends = new HashSet<>();
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement("SELECT u1.id as id1,u2.id as id2,u2.nume,u2.prenume from utilizatori u1 inner join prietenii p on u1.id=p.id1 inner join utilizatori u2 on p.id2=u2.id");
             ResultSet resultSet = statement.executeQuery()) {
